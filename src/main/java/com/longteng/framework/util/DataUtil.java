...
public class DataUtil {
	...
	public String getFilePath() {
		String filePath = this.getPath() + Config.get("DATA_EXCEL_PATH");
		return filePath;
	}
	...
	/**
	 * 获取对应场景senario下，对应用例名称casename的描述信息 用例表中的第一列CaseId + ‘，’ + 第四列Description
	 *
	 * @param senario
	 * @param caseName
	 * @return
	 ...
	 * 将excel中需要运行（RunFlag不为空且不为‘N’）的用例封装到LinkedHashMap<String,
	 * String>中，key为第二列TestCase，value为第一列+','+第四列描述
	 *
		...
		try {
			...
			for (int i = 1; i < sheet.getPhysicalNumberOfRows(); i++) {
				...
				if (row != null) {
					...
					String testDesc = "";
					try {
						testDesc = row.getCell(0).toString().trim() + Constants.Comma
